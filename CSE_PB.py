...
def optimal(X,S,A,n,m,c,sol,solbb,start_time):
    ...
    for idex in range(10):
        model = solve(test_solver, start_time = start_time)
        if model is None:
            print("no solution")
    ...
    while True:
        ...
        for t in range(c):
            ...
            for clause in pb_enc.clauses:
                solver.add_clause(clause)
                # print(stations)
def write_fancy_table_to_csv(ins, n, m, c, val, s_cons, h_cons, peak, status, time_elapsed, filename="pb_test.csv"):
    global best_result
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1 and sys.argv[1] in ['-h', '--help', 'help']:
        ...
        print("")
        print("Files created:")
        print("  Output/pb_test.csv   # CSV results")
    ...
    if len(sys.argv) == 1:
        ...
        for instance_id in range(0, len(file_name1)):
            ...
            command = f"./runlim -r {TIMEOUT} python3   pb_staircase4_time_test.py {instance_id}"
